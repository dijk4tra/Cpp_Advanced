...
void collect_items(tinyxml2::XMLNode* node, std::vector<tinyxml2::XMLElement*>& items)
{
    ...
    // C++17 允许在 if 条件中声明并初始化变量。
    // ToElement() 对非元素节点返回nullptr，指针转换为 bool 后决定是否进入分支。
...
{
    // 规则：max(5, min(200, text.size() / 120))
    // 即文本长度每满 120 字符取一个特征词，区间限制在 [5, 200]
    // 显式转为 int 以匹配 Simhasher 接口, 避免 size_t → int 可能触发警告
...
void PageProcessor::extract_documents(const std::string& dir)
{
    ...
    for (const auto& file : files) {
        ...
        if (err != tinyxml2::XML_SUCCESS) {
            // 单个 XML 损坏时跳过该文件并继续处理其他语料；
            // 日志保留文件路径，便于离线任务结束后定位并修复原始数据。
        ...
        for (tinyxml2::XMLElement* item : items) {
            // 要求：优先使用 content；没有 content 时使用 description；
            ...
            doc.content = content;
            // std::move 将 doc 转为右值，使 vector 可移动其 string 成员，
            // 避免复制正文等大字符串。移动后 doc 不再使用，并在本轮末尾正常析构。
...
void PageProcessor::deduplicate_documents()
{
    ...
    // uniqueDocuments 与 fingerprints 按相同下标一一对应，
    // 只为已经接受的非重复文档保存指纹。
    ...
    for (const auto& doc : documents_) {
        uint64_t hash = 0;
        // SimHash 将正文压缩成 64 位局部敏感指纹；相似文本倾向于拥有更小的汉明距离。
        // topN 随正文长度调整，避免长短文档使用同一关键词数量。
        // hash 通过引用参数写回；函数返回后保存正文对应的 64 位指纹。
        hasher_.make(doc.content, simhash_top_n(doc.content), hash);

        bool duplicated = false;
        // 将当前指纹与此前保留的每篇文档逐一比较。当前实现复杂度为 O(n^2)，
    ...
    // 阶段 2：swap 以常数复杂度交换两个 vector 的内部缓冲区。
    // documents_获得去重结果，旧原始文档随 uniqueDocuments 离开作用域自动释放。
                                            ...
                                            const std::string& offsets)
{
    ...
    // 网页库用二进制模式打开，避免平台换行转换影响 tellp() 和实际写入字节数的一致性。
    // 偏移库本身是按行读取的纯文本。
...
void PageProcessor::build_inverted_index(const std::string& filename)
{
    ...
    for (const auto& doc : documents_) {
        ...
        for (const auto& word : words) {
            ...
            // 连续两个 [] 分别访问外层 docId 和内层 word；
            // 不存在的映射会被默认构造，int 初始为 0，随后通过 ++ 累计。
    ...
    // 阶段 4：将倒排索引数据写入invert_index.dat。
