...
const int RetryMax = 3; //  MySQL 查询失败时最多重试 3 次
                     ...
                     const function<void(MySQLResultCursor&)>& handler)
{
    ...
    */
    bool ok = false;
    /*
        WaitGroup 用来把异步 MySQL 任务“当成”当前函数里的顺序流程。
    ...
    */
    WFMySQLTask* task = WFTaskFactory::create_mysql_task(
        ...
        [&](WFMySQLTask* task) {
            ...
            */
            ...
            /*
                把 cursor 交给调用方读取。
                注意 cursor 生命周期只在回调内有效，所以 handler 里要立即把需要的数据拷贝出来。
            */
            handler(cursor);
    ...
    task->get_req()->set_query(sql); // 把 SQL 写入 MySQL 请求
    ...
    */
    ...
    return ok; // 返回这次 SQL 是否顺利执行到 handler
}
