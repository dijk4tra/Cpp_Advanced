...
第二期的核心不是重新做离线建库，而是把一期产物组织成高效的在线查询服务。网络层保留 `muduo + TLV`，保证课程要求的 TCP 协议能力；同时增加内置 C++ HTTP 服务，方便浏览器直接访问 Web 前端页面。关键字推荐使用字符索引和编辑距离保证推荐结果相关；网页搜索使用 TF-IDF 和余弦相似度充分复用现有倒排索引；摘要使用动态窗口评分，让返回结果直接围绕用户查询词展示。
