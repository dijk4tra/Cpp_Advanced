...
1. 服务器框架：`muduo`。
2. 原始接口：自定义 `TLV` 协议，解决 TCP 粘包、半包问题，默认监听 `8888`。
3. 浏览器接口：内置 C++ 轻量 HTTP 服务，默认监听 `18888`，返回 `www` 前端页面和 HTTP API。
4. 关键字推荐算法：基于字符索引召回候选词，再用 UTF-8 感知的编辑距离、词频和字典序排序。
5. 网页搜索算法：基于倒排索引的 TF-IDF 向量空间模型，并使用余弦相似度排序。
6. 摘要生成方式：动态摘要，即根据查询关键词在正文中的位置和上下文生成摘要。
7. 前端页面：`Pandex / 关键字推荐&网页搜索`，风格参考 `docs/01-minimalism.html`。
...
flowchart TD
    ...
    subgraph Server[第二期在线服务器]
        Load[启动时加载数据]
        Muduo[muduo TcpServer TLV 8888]
        HTTP[WebHttpServer HTTP 18888]
        Codec[TLV 编解码]
        KW[关键字推荐模块]
        WS[网页搜索模块]
        ABS[动态摘要模块]
    ...
    Client[TLV 客户端] <-->|TLV over TCP| Muduo
    Browser[浏览器] <-->|HTTP| HTTP
    Muduo --> Codec
    Codec --> KW
    Codec --> WS
    HTTP --> KW
...
│       ├── SearchServer.h           # TLV 服务封装
│       ├── WebHttpServer.h          # 浏览器 HTTP 服务
...
当前实际实现中，`SearchServer` 负责原始 TLV 服务，`WebHttpServer` 负责浏览器页面和 HTTP API。两者运行在同一个 `search_server` 进程内，分别监听不同端口，共享启动时加载好的只读搜索数据。
...
服务器启动后，主线程运行 `EventLoop`，同一个进程内启动两个 `TcpServer`：

1. `SearchServer`：监听 `server_port`，默认 `8888`，处理原始 TCP/TLV 请求。
2. `WebHttpServer`：监听 `http_port`，默认 `18888`，处理浏览器页面和 HTTP API。

收到完整 TLV 消息后，`SearchServer` 根据消息类型分发到关键字推荐或网页搜索模块。浏览器访问 `http://127.0.0.1:18888` 时，`WebHttpServer` 返回 `www/index.html`、`styles.css`、`app.js`；浏览器提交搜索时，请求 `/api/suggest` 或 `/api/search`，HTTP 服务直接调用同进程内的业务模块。
...
sequenceDiagram
    ...
    participant H as WebHttpServer
    participant KW as KeywordRecommender
    participant WS as WebSearcher
    participant FS as www 静态文件

    B->>H: GET /
    H->>FS: 读取 index.html/styles.css/app.js
    H-->>B: 返回 Pandex 页面

    alt POST /api/suggest
        B->>H: JSON query/lang/topk
        H->>KW: recommend_json(...)
        KW-->>H: JSON 推荐结果
    else POST /api/search
        B->>H: JSON query/topk
        H->>WS: search_json(...)
        WS-->>H: JSON 搜索结果
    end
...
3. 使用 `server.setThreadNum(n)` 开启 TLV 服务 IO 线程。
4. 使用 `httpServer.setThreadNum(n)` 开启 HTTP 服务 IO 线程。
5. 所有词典、索引、网页偏移、倒排索引都作为只读数据共享，不在查询时修改。

只读共享数据天然适合多线程查询。如果后续加入缓存，缓存需要单独考虑加锁或使用线程局部缓存；但第二期当前实现不做缓存，避免复杂度过高。
...
1. 初始化另一个 `muduo::net::TcpServer`，监听 `http_port`。
2. 解析浏览器发来的最小 HTTP 请求。
3. 对 `GET /`、`GET /styles.css`、`GET /app.js` 返回 `www` 静态文件。
4. 对 `POST /api/suggest` 调用 `KeywordRecommender`。
5. 对 `POST /api/search` 调用 `WebSearcher`。

当前实现没有引入外部 HTTP 框架，只实现浏览器测试所需能力。这样仍然保持第二期代码简单，但用户只需要运行一个 `./bin/search_server`，即可同时使用 TLV 客户端和浏览器页面。
...
页面名称为 `Pandex`，主标题为 `关键字推荐&网页搜索`。视觉风格为minimalism / swiss 风格：白底黑字、细边框、留白充足、少动画和少装饰。

`WebHttpServer` 当前实现的是轻量 HTTP 服务，不是通用 Web 框架。它只支持本项目需要的请求：

```text
GET  /              -> www/index.html
GET  /styles.css    -> www/styles.css
GET  /app.js        -> www/app.js
POST /api/suggest   -> KeywordRecommender::recommend_json()
POST /api/search    -> WebSearcher::search_json()
```
...
3. 初始化 WebSearcher。
4. 创建 EventLoop。
5. 创建 SearchServer，监听 8888 TLV 端口。
6. 创建 WebHttpServer，监听 18888 HTTP 端口。
7. 启动 muduo 事件循环。
```
...
### 15.6 第六步：接入 WebHttpServer 和前端页面

TLV 服务验证通过后，再接入浏览器 HTTP 服务：

1. `GET /` 能返回 `www/index.html`。
2. `GET /styles.css` 和 `GET /app.js` 能返回静态资源。
3. `POST /api/suggest` 能返回推荐词 JSON。
4. `POST /api/search` 能返回网页搜索 JSON。
5. 浏览器访问 `http://127.0.0.1:18888` 能看到 Pandex 页面并完成查询。
...
8. 能通过 `http://127.0.0.1:18888` 访问 Pandex 前端页面。
9. HTTP API `/api/suggest` 和 `/api/search` 能复用同一套在线算法返回 JSON。

## 19. 总结

第二期的核心不是重新做离线建库，而是把一期产物组织成高效的在线查询服务。网络层保留 `muduo + TLV`，保证课程要求的 TCP 协议能力；同时增加内置 C++ HTTP 服务，方便浏览器直接访问 Pandex 前端页面。关键字推荐使用字符索引和编辑距离保证推荐结果相关；网页搜索使用 TF-IDF 和余弦相似度充分复用现有倒排索引；摘要使用动态窗口评分，让返回结果直接围绕用户查询词展示。

整体实现顺序建议为：

```text
数据加载 -> 关键字推荐 -> 网页搜索 -> 动态摘要 -> muduo/TLV 接入 -> WebHttpServer/前端接入 -> 集成测试
```
