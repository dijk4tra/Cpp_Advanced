...
`data/` 目录中 `dict/` 和 `index/` 的是一期最终交付的核心产物。二期服务器启动后，只需要加载这些文件，不应该再扫描原始语料。
...
其中 `is_chinese_word()` 专门服务于中文关键字推荐词典。它按 Unicode 码点判断 token 是否完全由汉字组成，而不是简单判断“不是标点就保留”。这样可以避免 `cppjieba` 把英文名、网址片段、圈号数字、罗马数字、特殊符号等内容切出来后写入 `dict_cn.dat`，导致中文词典和中文字符索引被污染。
...
3. 过滤停用词。
4. 只保留完全由汉字组成的 token，过滤英文、数字、标点、特殊符号以及中英混合 token。
5. 统计词频。
6. 写入 `dict_cn.dat`。
...
中文关键字推荐的词典应当保持“中文词典只负责中文候选词”的边界。英文候选词已经由 `dict_en.dat` 负责，因此 `dict_cn.dat` 不再保留英文、数字、特殊符号、圈号数字、罗马数字或中英混合 token。网页搜索倒排索引可以继续保留更宽松的分词结果，因为网页内容中可能存在英文关键词；这里的严格过滤只针对关键字推荐的中文词典和中文索引。
...
7. 中文词典只过滤标点和空白，没有过滤英文、数字和特殊符号，导致 `dict_cn.dat` 前面出现大量非中文 token。
...
1. `data/index/pages.dat`：保存去重后的网页文档原文。
2. `data/index/offsets.dat`：保存每篇网页在 `pages.dat` 中的位置，方便二期按 id 快速读取。
3. `data/index/invert_index.dat`：保存关键字到网页 id 和权重的映射，用于搜索排序。
...
2. 英文词典中是否没有纯数字、标点。
3. 停用词如 `the`、`is` 是否被过滤。
4. 中文词典中每个词是否都由汉字组成，不应出现英文、数字、罗马数字、圈号数字或特殊符号。
5. 中文索引中每个 key 是否是完整 UTF-8 汉字，而不是乱码字节或非中文字符。
...
```cpp
std::ifstream ifs("data/index/pages.dat", std::ios::binary);
ifs.seekg(offset);
...
| 中文词典混入非中文 token | `dict_cn.dat` 前若干行出现英文、数字或符号 | 中文词典生成阶段使用纯汉字判断，只保留全部码点都是汉字的 token |
...
- [ ] `data/dict_cn.dat` 已生成，词语全部由汉字组成，没有英文、数字和特殊符号。
- [ ] `data/index_cn.dat` 已生成，字符 key 是完整 UTF-8 汉字，没有非中文字符。
- [ ] `data/index/pages.dat` 已生成，每篇文档包含 `id/link/title/content`。
- [ ] `data/index/offsets.dat` 已生成，行数等于网页库文档数。
- [ ] `data/index/invert_index.dat` 已生成，格式为 `keyword docId weight ...`。
- [ ] 随机偏移读取能读出完整 `<doc>`。
