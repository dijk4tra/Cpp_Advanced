...
```bash
./tests/tlv_client --type keyword --query "搜索" --lang cn --topk 5
```

不带 `topk`，服务端使用 `conf/config.conf` 中的 `keyword_topk`：

```bash
./tests/tlv_client --type keyword --query "搜索" --lang cn
```
...
```bash
./tests/tlv_client --type web --query "汽车 召回" --topk 3
```

不带 `topk`，服务端使用 `conf/config.conf` 中的 `web_topk`：

```bash
./tests/tlv_client --type web --query "汽车 召回"
```
...
```bash
./tests/tlv_client --type 1 --raw-json '{"query":"搜索","lang":"cn","topk":5}'
```

网页搜索示例：

```bash
./tests/tlv_client --type 2 --raw-json '{"query":"汽车 召回","topk":3}'
```

也可以用这个方式测试异常请求：

```bash
./tests/tlv_client --type 2 --raw-json '{"query":""}'
```
